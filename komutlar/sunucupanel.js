...
exports.run = async (client, message, args) => {
  ...
  if (args[0] === "sıfırla") {
  if(!sunucupanel) return message.channel.send(emojiler.basarisiz + `Sunucu İstatistik Panelinin kurulu olduğuna emin misiniz? \n${emojiler.gold2} Sıfırlamak için\n${emojiler.gold1}\`!sunucupanel sıfırla\`\n yazınız.`);
...
db.set(`spanelDurum_${message.guild.id}`, 'acik')
 let durum = db.fetch(`spanelDurum${message.guild.id}`)
  if(!sunucupanel) return message.channel.send(emojiler.basarisiz + `Sunucu İstatistik Paneli zaten kurulu. \n${emojiler.gold2} Sıfırlamak için\n${emojiler.gold1}\`!sunucupanel sıfırla\`\n yazınız.`);
